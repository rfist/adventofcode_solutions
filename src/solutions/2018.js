module.exports = {
    1: {
        description: "--- Day 1: Chronal Calibration ---",
        solutions: [
            require('../puzzles/puzzle_2018_1_0'),
            require('../puzzles/puzzle_2018_1_1')
        ]
    },
    2: {
        description: "--- Day 2: Inventory Management System ---",
        solutions: [
            require('../puzzles/puzzle_2018_2_0'),
            require('../puzzles/puzzle_2018_2_1')
        ]
    },
    3: {
        description: "--- Day 3: No Matter How You Slice It ---",
        solutions: [
            require('../puzzles/puzzle_2018_3_0'),
            require('../puzzles/puzzle_2018_3_1')
        ]
    },
    4: {
        description: "--- Day 4: Repose Record ---",
        solutions: [
            require('../puzzles/puzzle_2018_4_0'),
            require('../puzzles/puzzle_2018_4_1'),
        ]
    }
};
